...
$conn->query("UPDATE news_provinces SET province= '$province'  , titre = '$titre'  WHERE id = $id");
...
foreach ($contenu_ids as $index => $contenu_id) {
    ...
    if ($images['error'][$index] == 0) {
        ...
        $sql = "UPDATE contenus_province SET paragraphe = '$paragraphe', image_path = '$image_path' WHERE id = $contenu_id";
    } else {
        $sql = "UPDATE contenus_province SET paragraphe = '$paragraphe' WHERE id = $contenu_id";
    }
...
echo "<script>
    ...
    window.location.href = 'gestion_news_provinces.php';
</script>";
?>
