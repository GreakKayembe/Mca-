...
$id = intval($_GET['id']);
$res = $conn->query("SELECT * FROM cadres WHERE id = $id");
$cad = $res->fetch_assoc();
...
<p style="margin-top:100px;">
    ...
    <title><?php echo $cad['titre']; ?></title>
  
<body class="bg-light">
<div class="container mt-4">
    <h2><?php echo htmlspecialchars($cad['titre']); ?></h2>
    <marquee class="text-muted"><p class="text-muted">Publication du <?php echo date('d/m/Y', strtotime($cad['date_publication'])); ?></p></marquee>
    <hr>

    <?php
    $contenus = $conn->query("SELECT * FROM contenus_cadres WHERE cadre_id = $id ORDER BY ordre");
    ...
    <div class="mt-4">
        ...
        <a class="btn btn-sm btn-outline-primary me-2" 
           href="https://twitter.com/intent/tweet?text=<?php echo urlencode($cad['titre']); ?>" 
           target="_blank"><i class="ti ti-brand-twitter"></i></a>

        <a class="btn btn-sm btn-outline-success me-2"
           href="https://wa.me/?text=<?php echo urlencode($cad['titre'] . "\n\n" . "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']); ?>"
