...
$conn->query("UPDATE actualites SET titre = '$titre' WHERE id = $id");
...
foreach ($contenu_ids as $index => $contenu_id) {
    ...
    if ($images['error'][$index] == 0) {
        ...
        $image_path = htmlspecialchars("uploads/" . uniqid() . "." . $ext);
        move_uploaded_file($images['tmp_name'][$index], $image_path);

        // Supprime l’ancienne image ?
        // Ajouter cette logique si tu veux (facultatif)

        $sql = "UPDATE actualite_contenus SET paragraphe = '$paragraphe', image_path = '$image_path' WHERE id = $contenu_id";
    } else {
        $sql = "UPDATE actualite_contenus SET paragraphe = '$paragraphe' WHERE id = $contenu_id";
    }
...
echo "<script>
    ...
    window.location.href = 'gestion_actualites.php';
</script>";
?>
