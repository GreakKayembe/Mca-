...
$id = intval($_GET['id']);
$res = $conn->query("SELECT * FROM diasporas WHERE id = $id");
$dia = $res->fetch_assoc();
...
<p style="margin-top:100px;">
    ...
    <title><?php echo $dia['titre']; ?></title>
  
<body class="bg-light">
<div class="container mt-4">
    <h2><?php echo htmlspecialchars($dia['titre']); ?></h2>
    <marquee class="text-muted"><p class="text-muted">Publication du <?php echo date('d/m/Y', strtotime($dia['date_publication'])); ?></p></marquee>
    <hr>

    <?php
    $contenus = $conn->query("SELECT * FROM contenus_diaspora WHERE diaspora_id = $id ORDER BY ordre");
    ...
    <div class="mt-4">
        ...
        <a class="btn btn-sm btn-outline-primary me-2" 
           href="https://twitter.com/intent/tweet?text=<?php echo urlencode($dia['titre']); ?>" 
        ...
        <a class="btn btn-sm btn-outline-success me-2"
           href="https://wa.me/?text=<?php echo urlencode($dia['titre'] . "\n\n" . "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']); ?>"
